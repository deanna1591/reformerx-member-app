  ...
  return (
    <div className="pb-28">
      <header className="rounded-b-[26px] bg-ink px-5 pb-6 pt-[max(1.2rem,env(safe-area-inset-top))] text-white">
        <h1 className="font-display text-[28px] uppercase tracking-wide">{t("miles.title")}</h1>
        <p className="mt-0.5 text-[13px] text-white/60">{t("miles.subtitle")}</p>
        ...
        <div className="mt-4 rounded-[22px] bg-white/10 p-5 text-center backdrop-blur">
          ...
          <p className="mt-1 text-[12px] uppercase tracking-wider text-white/60">{t("miles.totalClasses")}</p>
          {next ? (
            <>
              ...
              <p className="mt-2.5 text-[13px] text-white/75">
                <span className="font-semibold text-white">{t("miles.toGo", { n: toGo })}</span> {t("miles.untilClub", { n: next })}
              </p>
            </>
          ) : (
            <p className="mt-3 text-[13px] text-white/75">{t("miles.allUnlocked")}</p>
          )}
        </div>
      </header>
      ...
      <div className="px-5 pt-5">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          <div className="rounded-xl2 bg-card p-4 shadow-card">
            ...
            <p className="mt-1.5 text-[11px] uppercase tracking-wider text-smoke">{t("miles.longestStreak")}</p>
          </div>
          <div className="rounded-xl2 bg-card p-4 shadow-card">
            <p className="font-display text-[26px] leading-none tabular-nums">{records.bestMonth?.count ?? 0}</p>
            <p className="mt-1.5 text-[11px] uppercase tracking-wider text-smoke">{t("miles.bestMonth")}</p>
          </div>
          <div className="rounded-xl2 bg-card p-4 shadow-card">
            <p className="font-display text-[26px] leading-none tabular-nums">{act.challengesCompleted}</p>
            <p className="mt-1.5 text-[11px] uppercase tracking-wider text-smoke">{t("miles.challengesWon")}</p>
          </div>
          <div className="rounded-xl2 bg-card p-4 shadow-card">
        ...
        {next && (
          <div className="mt-4 rounded-xl2 bg-sage-soft p-5 text-center">
            <p className="font-display text-[18px]">{t("miles.clubClose", { n: next })}</p>
            <p className="mt-1 text-[13px] text-smoke">
              {t("miles.clubCloseBody", { n: toGo, classes: pluralClasses(locale, toGo) })}
            </p>
          </div>
        )}
      </div>
